 ...
 */
export default function smallTexts(doc: Document, fontSizeThreshold: number) {
    ...
    for (let i = 0, max = all.length; i < max; i += 1) {
        ...
        currentEl.removeAttribute('data-swds-smallTexts');
        ...
        if (text !== '' && parseInt(getComputedStyle(currentEl).fontSize, 10) < fontSizeThreshold) {
            currentEl.setAttribute('data-swds-smallTexts', '1');
            totalElements += 1;
